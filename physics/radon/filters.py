...
class AbstractFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        projection = (
            torch.fft.fft(padded_tensor.transpose(2, 3)).transpose(2, 3)
            * fourier_filter
        )
        return torch.fft.irfft(projection.transpose(2, 3)).transpose(2, 3)[
            :, :, :input_size, :
        ]
    ...
    def _get_fourier_filter(self, size):
        ...
        ff = torch.fft.fft(f)
        ff = torch.complex(ff.real, ff.real)
...
class LearnableFilter(AbstractFilter):
    def __init__(self, filter_size):
        ...
        self.filter = nn.parameter.Parameter(2 * fftfreq(filter_size).abs().view(-1, 1))

    def forward(self, x):
        fourier_filter = self.filter.unsqueeze(-1).repeat(1, 1, 2).to(x.device)
        projection = torch.fft.fft(x.transpose(2, 3)).transpose(2, 3) * fourier_filter
        return torch.fft.irfft(projection.transpose(2, 3)).transpose(2, 3)
